...
@app.route("/")
def health_check():
    return Response("Video Downloader Bot is Running", status=200, mimetype='text/plain')
    ...
    try:
        if COOKIES_BASE64:
            ...
            with open(COOKIES_FILE, "wb") as f:
                ...
                f.write(decoded)
            logger.info(f"✅ Created cookies file at {COOKIES_FILE} (Size: {len(decoded)} bytes)")
        elif Path(COOKIES_FILE).exists():
            logger.info("Using existing cookies file.")
        else:
            logger.warning("⚠️ No cookies file found and COOKIES_BASE64 not provided.")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("⚠️ Expired. Please send the link again.")
        return
    ...
    if file_path.stat().st_size > TELEGRAM_FILE_LIMIT:
        shutil.rmtree(temp_dir, ignore_errors=True)
        await query.edit_message_text("⚠️ File exceeds 2GB limit. Try a lower resolution.")
        return
...
def main():
    ...
    # Run Flask server in background (Render health check)
